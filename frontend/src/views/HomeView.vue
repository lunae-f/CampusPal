...
const shouldShowRow = (row) => {
  const yearMatch = selectedYears.value.length === 0 || (row.rishunen && selectedYears.value.includes(row.rishunen));
  const termMatch = selectedTerms.value.length === 0 || (row.syllabusData?.term && selectedTerms.value.includes(row.syllabusData.term));
  const categoryMatch = selectedCategories.value.length === 0 || (row.syllabusData?.category && selectedCategories.value.includes(row.syllabusData.category.split('・')[0]));
  const evalMatch = selectedEvaluations.value.length === 0 ||
          ...
          <div class="filter-group">
            ...
            <div class="checkbox-group">
              <div v-for="ev in availableEvaluations" :key="ev" class="checkbox-item">
                <input type="checkbox" :id="`eval-${ev}`" :value="ev" v-model="selectedEvaluations">
              ...
              <details class="filter-details">
                ...
                <div class="filter-options">
                  <div class="filter-group">
                    <label>年度</label>
                    <div class="checkbox-group">
                      <div v-for="year in availableYears" :key="`mobile-year-${year}`" class="checkbox-item">
                        <input type="checkbox" :id="`mobile-year-${year}`" :value="year" v-model="selectedYears">
                  ...
                  <div class="filter-group">
                    <label>開講時期</label>
                    <div class="checkbox-group">
                      <div v-for="term in availableTerms" :key="`mobile-term-${term}`" class="checkbox-item">
                        <input type="checkbox" :id="`mobile-term-${term}`" :value="term" v-model="selectedTerms">
                  ...
                  <div class="filter-group">
                    <label>分野系列</label>
                    <div class="checkbox-group">
                      <div v-for="cat in availableCategories" :key="`mobile-cat-${cat}`" class="checkbox-item">
                        <input type="checkbox" :id="`mobile-cat-${cat}`" :value="cat" v-model="selectedCategories">
                  ...
                  <div class="filter-group">
                    <label>評価</label>
                    <div class="checkbox-group">
                      <div v-for="ev in availableEvaluations" :key="`mobile-eval-${ev}`" class="checkbox-item">
                        <input type="checkbox" :id="`mobile-eval-${ev}`" :value="ev" v-model="selectedEvaluations">
...
.stats-container { display: grid; grid-template-columns: 1fr; gap: 20px; }
